...
export function hydrateActiveGallery(props, scope) {
  ...
  const galleries = props.customerGalleries.images;
  const categories = props.customerGalleries.categories;
  ...
  if (Object.keys(categories).length > 0) {
    ...
    if (path !== galleryPath) { // if needed correct browser url to show current category
      scope.context.router.replace(`/customer-galleries/${galleryPath}`);
    }
    ...
    forIn(activeGallery, (image, id) => {
      gallery[id] = {
        ...
        show: !scope.props.customerGalleries.seqImagesLoadedEnabled ? true : false // since that function would otherwise reveals images
      };
    });

    scope.setState({ ...scope.state, gallery, activeGalleryId });
    if (!scope.props.customerGalleries.forceImagesLoadedOff) {
...
export function seqImagesLoaded(element, scope) {
  const imgLoad = imagesLoaded(element);
  if (scope.props.customerGalleries.seqImagesLoadedEnabled) {
