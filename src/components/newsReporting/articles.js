    ...
    if (element) {
      ...
      elements.push(
        <div key={id} className="article" data-textedittargetparent>
          ...
          {authenticated && scope.state.isEditing ? <div><label htmlFor={`article__src_input-${id}`}>Story Link</label><input type="text" id={`article__src_input-${id}`} className="article__src_input" placeholder={src} data-articleid={id} defaultValue={src} onChange={scope.onArticleSrcChange}/></div> : null}
