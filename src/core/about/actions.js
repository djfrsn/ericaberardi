...
export function editAbout(opts) {
  return (dispatch, getState) => {
    ...
    // database.ref('about/articles').set(articles);
    ...
    //         const id = firebase.database('about/articles').ref().child(`${article.id}`).push().key;
    ...
    //         // update meta
    //         database.ref(`about/articles/${article.id}`).set(newArticle);
    //       });
    //     }
    //   }
    // });
...
export function deleteAboutFile(prevArticle) {
  return (dispatch, getState) => {
    const { firebase } = getState();
    const storage = firebase.storage();
